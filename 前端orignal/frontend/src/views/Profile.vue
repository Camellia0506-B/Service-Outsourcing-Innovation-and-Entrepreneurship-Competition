...
// 获取用户信息（符合 API 文档 2.1：profile/info）
const fetchUserInfo = async () => {
    try {
        ...
        if (res.code === 200 && res.data) {
            // 根据API文档2.1，profile/info返回完整的个人档案信息
            userInfo.value = {
                id: res.data.user_id,
                username: res.data.basic_info?.nickname || res.data.basic_info?.username || '',
                nickname: res.data.basic_info?.nickname || '',
                avatar: res.data.basic_info?.avatar || ''
            }
            stats.value = res.data.stats || { follow_count: 0, post_count: 0, resource_count: 0 }
            editForm.value.nickname = res.data.basic_info?.nickname || ''
        }
    } catch (error) {
...
// 更新个人资料（符合 API 文档 2.2 profile/update）
...
// 退出登录（符合 API 文档 1.3：auth/logout）
const handleLogout = async () => {
    if (!confirm('确定要退出登录吗？')) return
    try {
        const userId = localStorage.getItem('user_id')
        if (userId) await logoutAPI(Number(userId))
    } catch (e) {
