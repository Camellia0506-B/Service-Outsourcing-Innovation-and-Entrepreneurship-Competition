...
职业测评模块 - 服务层
对应 API 文档第 3 章
架构：RAG 管理题库(ChromaDB) → 动态抽题 → 用户答题 → 异步任务生成报告(qwen3-max)
1. 动态问卷：从 ChromaDB 向量检索按维度抽题，题目不写死在前端
2. 多维度计分：霍兰德/MBTI/能力/价值观
3. 异步报告：提交后后台线程调用 qwen3-max 生成个性化报告，前端轮询 report_id
"""
