...
**功能说明**：学生开始投递某个推荐岗位后，创建该岗位的求职闭环记录。
...
**方法**：`DELETE`\
**路径**：`/tracking/failure-reports/<report_id>`
...
**请求体（JSON）**：
  ...
  参数名            类型              必填              说明
  ----------------- ----------------- ----------------- -----------------
  user_id           number/string     是                用户ID（须与报告归属一致）
  -----------------------------------------------------------------------

**说明**：删除成功后，若报告关联了求职闭环记录（`record_id`），会将该记录的 `has_failure_report` 置为 `false`。

**响应示例**：

    { "code": 200, "msg": "报告已删除", "data": null }
...
本系统使用以下AI模型：
  ...
  简历解析                            专用NLP模型 + Qwen-plus

  人岗匹配（语义）                    BGE-M3向量模型 + ChromaDB / Milvus

  人岗匹配（融合）                    语义相似度 × 0.6 + 字段匹配 × 0.4

  智能体对话                         Qwen-plus（ReAct模式，支持工具调用）

  模拟面试评估                        Qwen-plus + 自研评分模型

  职业规划报告生成                    Qwen-plus（长文本，流式）

  知识库RAG                           text-embedding-v4 + Qwen-plus（RAG增强）
  --------------------------------------------------------------------------
