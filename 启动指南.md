...
在项目根目录打开终端执行（推荐 PowerShell / CMD 均可）：

```bash
python -m pip install -r requirements.txt
```

> 说明：`requirements.txt` 已自动指向 `requirements-py310.txt`（兼容 Python 3.10 的 Windows 环境）。

### 1. 启动 AI 服务

1. **再开一个窗口**：双击 `start-ai.bat`
2. 看到 AI 服务在 **http://127.0.0.1:5002** 启动成功后，前端里的职业测评、岗位画像、职业规划报告、求职跟踪等会连到该服务
3. 若 `frontfiles/api.js` 里 `mockMode` 为 `true`，可改为 `false` 以使用真实接口

### 2. 验证 AI 服务是否正常（排查 404/500 很有用）

- 健康检查：`http://127.0.0.1:5002/api/v1/health`
- 查看已注册路由（用于确认接口是否存在）：`http://127.0.0.1:5002/api/v1/routes`

### 3. 简历解析（上传 PDF 填充档案）

- **简历解析**也走 AI 服务 5002（`/profile/upload-resume`、`/profile/resume-parse-result`）。若上传后提示「未识别到可填充项」：
  1. 确认 **AI 服务已启动**（见上文 1），且 5002 端口可访问；
  2. 确认 **PDF 为可复制文本**（非扫描件/图片 PDF），否则无法提取文字；
  3. 可尝试重新上传或换一份简历测试。
...
看到类似 **`Tomcat started on port(s): 5000`** 即表示成功。

建议用健康检查接口验证（而不是直接访问 `/api/v1` 根路径）：
...
2. **启动 Java 后端（5000）**
3. **启动 AI 服务（5002）**（需要测评/岗位画像/职业规划时）
...
| AI 算法  | 5002 | 职业测评、岗位画像、职业规划报告（默认 5002，避免 Windows 保留 5000-5001） |
...
- 首次安装依赖：`Set-Location "d:\Projects\Service-Outsourcing-Innovation-and-Entrepreneurship-Competition"; python -m pip install -r requirements.txt`
...
> 如果你的终端/命令行对中文路径显示乱码，**优先双击 `start-ai.bat`** 启动 AI 服务，避免手动输入 `AI算法` 目录导致路径不一致。

如果你必须用命令行启动，但 `AI算法` 目录在终端里显示乱码/无法进入，可用下面这条（不需要手动输入中文路径）：

```bash
python -c "import os,runpy; root=r'd:\Projects\Service-Outsourcing-Innovation-and-Entrepreneurship-Competition'; d=next(x for x in os.listdir(root) if x.lower().startswith('ai')); os.chdir(os.path.join(root,d)); runpy.run_path('app.py', run_name='__main__')"
```
...
- 先运行 `start-ai.bat` 启动 AI 服务（默认端口 **5002**）
...
### 9. AI 服务启动报错：“以一种访问权限不允许的方式做了一个访问套接字的尝试”

- 这是 **Windows 保留了 5000/5001 等端口** 导致的，本项目的 AI 服务已改为默认使用 **5002** 端口
- 直接重新运行 `start-ai.bat` 或 `cd AI算法 && python app.py` 即可，前端已配置为连 5002
- 若需改用其他端口：先设置环境变量 `set AI_SERVICE_PORT=6002`，再启动 AI 服务，并把 `frontfiles/api.js` 里的 `assessmentBaseURL`、`jobProfilesBaseURL` 中的端口改为 6002

### 10. 5002 端口一直拒绝连接（测评/岗位画像/职业规划都不可用）

- AI 服务没有启动，或启动后报错退出
- 在 `AI算法` 目录运行：`python app.py`
- 看到 **`Running on http://127.0.0.1:5002`**（或日志里写明的端口）才算启动成功；否则请查看窗口里的 Traceback 报错并按提示修复

### 11. 控制台出现 404/500（例如 recommend-jobs / assessment/report-history / career/report-history）

- **先确认 AI 服务是否真的启动**：打开 `http://127.0.0.1:5002/api/v1/health` 应返回 200
- **确认接口是否存在**：打开 `http://127.0.0.1:5002/api/v1/routes`，检查列表里是否包含你访问的路径
- **一直卡住不返回**：
  - 若你没有配置大模型 Key（`DASHSCOPE_API_KEY`），AI 服务会自动降级走规则生成（不影响基本功能），但首次匹配/索引构建可能需要几十秒
  - 建议先用浏览器强刷：**Ctrl + F5**，避免旧版前端脚本缓存导致请求打到错误端口/地址
